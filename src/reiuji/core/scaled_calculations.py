...
def divide(model: cp_model.CpModel, target: cp_model.IntVar, a: cp_model.IntVar | int, b: cp_model.IntVar | int) -> None:
    ...
    """
    op_id = uuid.uuid4()
    scaled_numerator = model.NewIntVar(cp_model.INT32_MIN, cp_model.INT32_MAX, f"{op_id}_scaled_numerator")
    ...
    b_pos = model.NewIntVar(1, cp_model.INT32_MAX, f"{op_id}_b_pos")
    model.Add(b_pos == b).OnlyEnforceIf(is_positive)
    b_neg = model.NewIntVar(cp_model.INT32_MIN, -1, f"{op_id}_b_neg")
    model.Add(b_neg == b).OnlyEnforceIf(is_positive.Not())
    target_pos = model.NewIntVar(cp_model.INT32_MIN, cp_model.INT32_MAX, f"{op_id}_target_pos")
    target_neg = model.NewIntVar(cp_model.INT32_MIN, cp_model.INT32_MAX, f"{op_id}_target_neg")
...
def sqrt(model: cp_model.CpModel, target: cp_model.IntVar, a: cp_model.IntVar, *, iter: int = 10) -> None:
    ...
    guess = a
    for i in range(iter):
        q = model.NewIntVar(cp_model.INT32_MIN, cp_model.INT32_MAX, f"{op_id}_q_{i}")
        divide(model, q, a, guess)
        s = model.NewIntVar(cp_model.INT32_MIN, cp_model.INT32_MAX, f"{op_id}_s_{i}")
        model.Add(s == guess + q)

        new_guess = model.NewIntVar(cp_model.INT32_MIN, cp_model.INT32_MAX, f"{op_id}_guess_{i + 1}")
